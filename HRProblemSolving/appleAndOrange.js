...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
  ...
  const landingApples = apples
    .map((apple) => apple + a)
    .filter((apple) => apple >= s && apple <= t);
  const landingOranges = oranges
    .map((orange) => orange + b)
    .filter((orange) => orange >= s && orange <= t);

  if (landingApples) console.log(landingApples.length);
  if (landingOranges) console.log(landingOranges.length);
}
// Solution end
